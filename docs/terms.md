...
| verified | For a candidate, the kernel status in which every required verifier recorded `PASS`; the solver reads it as evidence and decides nothing by it. For a note, the projection's flag: correctness and source passed it in one verification or it carries external verification, every note in its support is verified, and it is not dead. |
...
| verdict | `verifier`, `note`, `PASS`, `FAIL`, or `INCONCLUSIVE`, `report`, recorded on the note it names, always a note under verification. `FAIL` requires a concrete defect or an unmet completion criterion; `INCONCLUSIVE` means the check could not settle the claim and identifies what remains unresolved. |
