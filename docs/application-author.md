...
Large provider payloads can be saved explicitly with `campaign.storePayload(value)`. It returns a content digest, and `reader.payload(digest)` reconstructs the captured JSON, including property order. Identical payloads share one row. Put the digest in a versioned application request rather than inventing magic reference keys that ordinary entry reads must interpret. Saving a payload is local persistence and makes no provider request.
