...
Set `terminalTool` to the submission tool name when the same call also exposes observation tools. Successful observation tools return their results into the conversation. A successful batch containing only the named terminal tool ends the call. Without this option, every successful tool batch ends the call. With `submissionGate`, the gate instead decides which valid submission ends the call and requires a sole tool.
