...
export const sourceAssessment =
  "Open and read the cited paper or another authoritative primary source for every listed result. Locate the actual theorem and check its hypotheses, conclusion, and problem variant against the note. Search snippets, abstracts that do not state the needed result, a plausible citation, and your recollection cannot replace this check. Record a source entry for each result inspected: result repeats its externalResults entry exactly, source identifies the paper and theorem or section, url identifies the page you opened, and quote gives the relevant passage from that source. Sources may also document a mismatch. PASS requires retrieved evidence establishing every listed result and its applicability. If a necessary source or statement cannot be inspected, return INCONCLUSIVE and identify the unresolved result; do not fall back to recollection. Fail for a concrete false statement, source mismatch, or incorrect application.";
