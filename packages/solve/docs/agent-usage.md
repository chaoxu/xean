...
Guidance and submitted notes are already in the campaign. Leave the original settings file unchanged. The command resumes the first missing role call and preserves completed work. A fresh campaign receives ten turns by default. `run --turns N` or `init --turns N` sets its initial allowance outside the settings.
