...
Set `"maxExplorerResponses": 5` in settings to choose the response budget. The default is 5 responses per Explorer call, including the first, so it permits at most four continuation pushes. A value of 1 hands off after the first response; a large value such as 1000000000000000 leaves the context budget as the practical limit. This bounds responses, not reasoning tokens or cost. Plain text, length-limited output, and rejected submissions count. Provider errors do not count and use the separate recovery allowance. The setting applies only with continuation enabled and is frozen with the campaign settings. A fresh role call after failure or interruption starts a fresh budget; internal provider retries share the existing count.
