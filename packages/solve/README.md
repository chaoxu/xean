...
`run --turns 10` grants the initial allowance of ten Explorer turns. Omitting `--turns` gives a fresh campaign ten turns. The allowance lives in journal records, outside the frozen task and model settings. After a campaign reaches `turn-limit`, grant another ten turns and resume it:
