...
New artifacts use SQLite schema 6. Older schemas remain immutable historical artifacts and require their matching runtime. Opening an older artifact does not migrate it.
...
`Campaign.storePayload(value)` returns the SHA-256 digest of the JSON-serialized payload. `Reader.payload(digest)` reconstructs those JSON serialization semantics, including property order. The `payloads` table stores the non-input fields and an ordered digest array. The `payload_items` table stores each distinct top-level input item once. Payloads without an input array store their complete JSON directly. Literal reference-like keys are ordinary data. The storage relation, rather than a reserved key inside user JSON, identifies stored input items. Reads reject missing items, invalid manifests, item digest mismatches, and reconstructed payload digest mismatches. Payload tables are append-only. A failed save rolls back its new items and manifest together.
